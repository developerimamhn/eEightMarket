...
const BtnActiveBg = ({ id = "btn" }) => {
  const p0 = `paint0_${id}`;
  const p1 = `paint1_${id}`;
  const p2 = `paint2_${id}`;

  return (
    <svg className="absolute inset-0 w-full h-full" viewBox="0 0 223 48" preserveAspectRatio="none" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <div className="w-full px-3 sm:px-5 py-3">
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-0">
        <div className="flex items-center self-start sm:self-auto gap-2">
          <img src={logo} alt="logo" className="object-contain h-11 rounded-2xl" />
          <span className="user text-[14px] sm:text-[16px] lg:text-[18px] text-white leading-[160%]">
            User Setting
          </span>
        </div>
        <div className="flex items-center p-1 rounded-2xl w-full sm:w-auto bg-white/[0.05] border border-white/10">
          {tabs.map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`relative flex flex-1 sm:flex-none justify-center items-center px-3 sm:px-4 py-2 sm:py-[9px] text-[11px] sm:text-[14px] whitespace-nowrap rounded-[14px] leading-[150%] min-w-0 transition-all duration-200 ${activeTab === tab ? "text-white" : "text-white/50 hover:text-white/80"}`}
            >
              {activeTab === tab && (
                <div className="absolute inset-0 z-0 pointer-events-none overflow-hidden rounded-[14px]">
                  <BtnActiveBg id={tab} />
                </div>
              )}
