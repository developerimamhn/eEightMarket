        ...
        <div className="flex gap-[6px] sm:gap-[8px] flex-wrap">
          {sources.map((src) => (
            <button
              ...
              className={`flex items-center gap-[6px] sm:gap-[8px] px-[8px] sm:px-[12px] py-[6px] sm:py-[7px] rounded-[10px] border text-[11px] sm:text-[12px] transition
              ${activeSource === src
                  ? "bg-white/10 border-white/30 text-white"
                  : `${sourceBadgeBorder[src]} bg-white/5 text-white/60 hover:bg-white/10`
                }`}
            >
              <img src={tikIcon} className="h-[16px] sm:h-[20px]" alt="" />
              <img src={sourceLogos[src]} className="h-[10px] sm:h-[12px]" alt={src} />
            </button>
          ))}
        </div>
      </div>
